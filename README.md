...
Este projeto GoodHamburger faz parte de um processo seletivo. Para mim, foi uma ótima oportunidade para revisar conteúdos de base.

O projeto consiste em:
- Web API em .NET 10
- Blazor App (modo Server)

Os perfis de lançamento já estão preparados para facilitar a execução do código no Visual Studio 2022 ou superior.

### Login e acesso

A API possui autenticação e autorização. Dois usuários já estão pré-configurados:
...
O portal Blazor consome a API e utiliza o mesmo sistema de autenticação. Ao executar ambos os projetos, basta fazer login com um dos usuários acima.
